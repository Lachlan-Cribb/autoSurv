...
#' @param id optional vector of id's for stratifying cross-validation
...
autoSurv <- function(
  ...
  if (testProp < 1 & testProp > 0) {
    if (is.null(id)) {
      ...
      testIDs <- sample(
        x = uniqueIDs,
        size = ceiling(length(uniqueIDs) * testProp)
      )
      testDat <- data[id %in% testIDs, ]
      trainDat <- data[!id %in% testIDs, ]
      trainIDs <- id[!id %in% testIDs]
    }
  } else if (testProp == 0) {
    warning(
      "testProp is set to 0. Performance results will be measured
  ...
  if (cv) {
    if (cvFold > 1) {
      if (is.null(id)) {
        ...
        internal_fold <- sample(
          rep_len(1:cvFold, length.out = nrow(trainDat)),
          nrow(trainDat)
        )
      } else {
        set.seed(seed)
        uniqueIDs <- unique(trainIDs)
        folds <- sample(rep_len(1:cvFold, length.out = length(uniqueIDs)))
        folds <- setNames(folds, uniqueIDs)
        internal_fold <- folds[trainIDs]
      }
    } else {
  ...
  if (is.null(times)) {
    times <- quantile(
      trainDat[[timeVar]][trainDat[[statusVar]] == 1],
      0.5,
      names = FALSE
    )
  }
  ...
  if ("cox" %in% trainModels) {
    cox_cv <- function() {
      ...
      for (i in seq_len(cvFold)) {
        ...
        coxphModel <- survival::coxph(.form, data = fold_train, x = TRUE)
        brier <- NULL
        temp_Score <- riskRegression::Score(
          list("Cox" = coxphModel),
          formula = .form_cens,
          data = fold_test,
          times = times,
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "Cox"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "Cox"
          ]
        }
        ...
        scoreBrier$R2 <- 1 -
          scoreBrier$Brier /
            scoreBrier$Brier[which(scoreBrier$model == "Null model")]
        scoreBrier$R2_IBS <- 1 -
          scoreBrier$IBS /
            scoreBrier$IBS[which(scoreBrier$model == "Null model")]
      ...
      list(Score = neg_brier, Pred = scoreCV)
    }

    OPT_Res_cox <- cox_cv()

    coxphModel <- survival::coxph(.form, data = trainDat)
  ...
  if ("rsf" %in% trainModels) {
    rf_cv <- function(nodesize, mtry) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        rfsrcModel <- randomForestSRC::rfsrc(
          .form,
          nodesize = nodesize,
          mtry = mtry,
          data = fold_train
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "RSF"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "RSF"
          ]
        }
    ...
    cvResults$rsf <- OPT_Res$Pred[[head(
      which(OPT_Res$History$Value == OPT_Res$Best_Value),
      1
    )]]
  ...
  if ("glm" %in% trainModels) {
    glm_cv <- function(intervals) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "GLM"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "GLM"
          ]
        }
        fold_briers <- c(fold_briers, brier)
    ...
    OPT_Res_GLM <- rBayesianOptimization::BayesianOptimization(
      glm_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper))
      ),
      init_grid_dt = NULL,
      init_points = init,
      n_iter = iter,
    ...
    cvResults$glm <- OPT_Res_GLM$Pred[[head(
      which(OPT_Res_GLM$History$Value == OPT_Res_GLM$Best_Value),
      1
    )]]
  ...
  if ("gbm" %in% trainModels) {
    gbm_cv <- function(
      ...
      for (i in seq_len(cvFold)) {
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "GBM"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "GBM"
          ]
        }
    ...
    OPT_Res_GBM <- rBayesianOptimization::BayesianOptimization(
      gbm_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper)),
        n.trees = c(50L, 500L),
    ...
    preds_GBM <- predBinSurv(
      testDat = testDat,
      delta.upper = OPT_pred.GBM[["delta.upper"]],
      model = OPT_pred.GBM[["model"]],
      times
    )
    ...
    cvResults$gbm <- OPT_Res_GBM$Pred[[head(
      which(OPT_Res_GBM$History$Value == OPT_Res_GBM$Best_Value),
      1
    )]]
  ...
  if ("glmnet" %in% trainModels) {
    glmnet_cv <- function(intervals, alpha, lambda) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        pred.glmnet.mat <- predBinSurv(
          testDat = fold_test,
          delta.upper = pred.glmnet[["delta.upper"]],
          model = pred.glmnet[["model"]],
          times
        )

        temp_Score <- riskRegression::Score(
          list("glmnet" = pred.glmnet.mat),
          formula = .form_cens,
          data = fold_test,
          times = times,
          summary = "ibs"
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[
              temp_Score$Brier$score$model == "glmnet"
            ],
            1
          )
        } else {
    ...
    OPT_Res_glmnet <- rBayesianOptimization::BayesianOptimization(
      glmnet_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper)),
        alpha = c(0, 0.5),
    ...
    OPT_pred.glmnet <- glmnetBinModel(
      trainDat = trainDat,
      timeVar = timeVar,
      statusVar = statusVar,
      w = w_pred,
    ...
    preds_glmnet <- predBinSurv(
      testDat = testDat,
      delta.upper = OPT_pred.glmnet[["delta.upper"]],
      model = OPT_pred.glmnet[["model"]],
      times
    )
    ...
    cvResults$glmnet <- OPT_Res_glmnet$Pred[[head(
      which(OPT_Res_glmnet$History$Value == OPT_Res_glmnet$Best_Value),
      1
    )]]
  ...
  if ("svm" %in% trainModels) {
    svm_cv <- function(intervals, C, sigma) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        pred.svm.mat <- predBinSurv(
          testDat = fold_test,
          delta.upper = pred.svm[["delta.upper"]],
          model = pred.svm[["model"]],
          times
        )

        temp_Score <- riskRegression::Score(
          list("svm" = pred.svm.mat),
          formula = .form_cens,
          data = fold_test,
          times = times,
          summary = "ibs"
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "svm"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "svm"
          ]
        }
        fold_briers <- c(fold_briers, brier)
    ...
    OPT_Res_svm <- rBayesianOptimization::BayesianOptimization(
      svm_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper)),
        C = c(0, 30),
    ...
    preds_svm <- predBinSurv(
      testDat = testDat,
      delta.upper = OPT_pred.svm[["delta.upper"]],
      model = OPT_pred.svm[["model"]],
      times
    )
    ...
    cvResults$svm <- OPT_Res_svm$Pred[[head(
      which(OPT_Res_svm$History$Value == OPT_Res_svm$Best_Value),
      1
    )]]
  ...
  if ("cforest" %in% trainModels) {
    cforest_cv <- function(intervals, mtry) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        pred.cforest.mat <- predBinSurv(
          testDat = fold_test,
          delta.upper = pred.cforest[["delta.upper"]],
          model = pred.cforest[["model"]],
          times
        )

        temp_Score <- riskRegression::Score(
          list("cforest" = pred.cforest.mat),
          formula = .form_cens,
          data = fold_test,
          times = times,
          summary = "ibs"
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[
              temp_Score$Brier$score$model == "cforest"
            ],
            1
          )
        } else {
    ...
    OPT_Res_cforest <- rBayesianOptimization::BayesianOptimization(
      cforest_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper)),
        mtry = c(2L, as.integer(ncol(trainDat) - 2))
    ...
    OPT_pred.cforest <- cforestBinModel(
      trainDat = trainDat,
      timeVar = timeVar,
      statusVar = statusVar,
      w = w_pred,
    ...
    cvResults$cforest <- OPT_Res_cforest$Pred[[head(
      which(OPT_Res_cforest$History$Value == OPT_Res_cforest$Best_Value),
      1
    )]]
    cvMetrics$cforest <- Reduce("+", cvResults$cforest) /
      length(cvResults$cforest)
  }
  ...
  if ("nnet" %in% trainModels) {
    nnet_cv <- function(intervals, size, decay) {
      ...
      for (i in seq_len(cvFold)) {
        ...
        if (length(times) > 1) {
          brier <- tail(
            temp_Score$Brier$score$IBS[temp_Score$Brier$score$model == "nnet"],
            1
          )
        } else {
          brier <- temp_Score$Brier$score$Brier[
            temp_Score$Brier$score$model == "nnet"
          ]
        }
        fold_briers <- c(fold_briers, brier)
    ...
    OPT_Res_nnet <- rBayesianOptimization::BayesianOptimization(
      nnet_cv,
      bounds = list(
        intervals = c(as.integer(bins.lower), as.integer(bins.upper)),
        size = c(1L, 10L),
    ...
    preds_nnet <- predBinSurv(
      testDat = testDat,
      delta.upper = OPT_pred.nnet[["delta.upper"]],
      model = OPT_pred.nnet[["model"]],
      times
    )
    ...
    cvResults$nnet <- OPT_Res_nnet$Pred[[head(
      which(OPT_Res_nnet$History$Value == OPT_Res_nnet$Best_Value),
      1
    )]]
  ...
  scoreBrier <- temp_Score$Brier$score
  scoreBrier$R2 <- 1 -
    scoreBrier$Brier / scoreBrier$Brier[which(scoreBrier$model == "Null model")]
  scoreBrier$R2_IBS <- 1 -
    scoreBrier$IBS / scoreBrier$IBS[which(scoreBrier$model == "Null model")]

  scoreALL <- data.frame(
    model = as.character(scoreAUC$model),
    AUC = scoreAUC$AUC,
    Brier = scoreBrier$Brier[-which(scoreBrier$model == "Null model")],
    R2 = scoreBrier$R2[-which(scoreBrier$model == "Null model")],
    IBS = scoreBrier$IBS[-which(scoreBrier$model == "Null model")],
...
createDiscreteDat <- function(
  ...
  if (cens == "same") {
    ...
    long.dat$N.delta <- ifelse(
      long.dat$timeInternal > long.dat$delta.upper,
      0,
      ifelse(
        long.dat$eventInternal == 1,
        ifelse(long.dat$timeInternal <= long.dat$delta.lower, NA, 1),
    ...
    long.dat$N.delta <- ifelse(
      ...
      ifelse(
        ...
        ifelse(long.dat$timeInternal <= long.dat$delta.lower, NA, 1),
        ifelse(
          long.dat$timeInternal >=
            0.5 * (long.dat$delta.upper + long.dat$delta.lower),
          0,
          NA
        )
      )
  ...
  long.dat$N.delta <- ifelse(
    long.dat$timeInternal == m & long.dat$delta.upper == m,
    ifelse(is.na(long.dat$N.delta), 0, long.dat$N.delta),
    long.dat$N.delta
  )
...
genPTdat <- function(
  ...
  delta.upper <- unique(createDiscreteIntervals(
    time = dat_i[, time.ind],
  ...
  dat_i.X <- createDiscreteDat(
    timeInternal = dat_i[, time.ind],
...
predBinSurv <- function(testDat, delta.upper, model, times) {
  ...
  if (model$method %in% c("svmRadial", "cforest", "nnet")) {
    testDat.pt$pred <- predict(model, newdata = testDat.pt, type = "prob")[,
      "yes"
    ]
  } else {
    testDat.pt$pred <- predict(model, newdata = testDat.pt, type = "prob")[,
...
glmBinModel <- function(
  ...
  return(list(delta.upper = unique(ptData[["delta.upper"]]), model = modelGlm))
}


# Function for prediction from a GAM --------------------------------------
gamBinModel <- function(
  trainDat,
  timeVar = timeVar,
  statusVar = statusVar,
  w,
  ...
  return(list(delta.upper = unique(ptData[["delta.upper"]]), model = modelGam))
}


# Function for prediction from a glmnet -----------------------------------
glmnetBinModel <- function(
  trainDat,
  timeVar = timeVar,
  statusVar = statusVar,
  w,
  ...
  return(list(
    delta.upper = unique(ptData[["delta.upper"]]),
    model = modelGlmnet
  ))
}
...
gbmBinModel <- function(
  ...
  return(list(delta.upper = unique(ptData[["delta.upper"]]), model = modelGbm))
}


# Function for prediction from a SVM --------------------------------------
svmBinModel <- function(
  trainDat,
  timeVar = timeVar,
  statusVar = statusVar,
  w,
  ...
  return(list(delta.upper = unique(ptData[["delta.upper"]]), model = modelSvm))
}


# Function for prediction from a conditional inference forest -------------
cforestBinModel <- function(
  trainDat,
  timeVar,
  statusVar,
  w,
...
nnetBinModel <- function(
  ...
  return(list(delta.upper = unique(ptData[["delta.upper"]]), model = modelNnet))
}
